...
class _FakeDeviceModule:
    """The accelerator surface capture() uses: streams, a graph pool, a scope."""
    ...
    def Stream(self, device: torch.device) -> _FakeStream:  # noqa: N802
        del device
...
def _stub_vocoder_build(monkeypatch, flow: SimpleNamespace) -> list[_RecordingRunner]:
    """Reduce create_vocoder_executor to the Flow-graph decision."""
    monkeypatch.setattr(
