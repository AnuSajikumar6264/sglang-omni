...
"""What the LLaDA2-Uni MoE block owes its tensor-parallel peers.

The routed experts are sharded across TP ranks, so each rank's FusedMoE returns
a partial sum -- nothing at all for a token routed entirely elsewhere. Summing
that with the shared expert and returning it leaves every rank holding a
different hidden state, and because the divergence enters at the first MoE layer
it compounds through the remaining nineteen: the 16B thinker served fluent
nonsense, then empty completions, with no error anywhere. Nothing catches this
at tp_size=1, which is how the block shipped.

The real collectives need a process group, so the block is driven here with its
parallel layers faked and the all-reduce observed directly.
"""
...
class _FakeExperts(nn.Module):
    """Stands in for FusedMoE, returning one rank's partial sum."""
...
class _FakeRowParallel(nn.Module):
    """Records the reduce_results it was built with, as the caller's contract."""
...
def test_the_moe_block_all_reduces_its_partial_sum_under_tp(
    ...
    """Without this the ranks disagree from the first MoE layer onward."""
...
def test_the_shared_expert_does_not_reduce_its_own_output(
    ...
    """It is summed into the routed partial sum before the single all-reduce, so
    reducing inside its down_proj as well would count it tp_size times."""
...
def test_a_single_rank_owns_every_expert_and_stays_off_the_collective(
    ...
    """tp_size=1 holds the whole sum already; an all-reduce would only cost a
    launch. This is the path the block was written for and still has to work."""
...
def test_a_downstream_reduce_suppresses_this_one(
    ...
    """A fused or scattered reduce downstream absorbs this all-reduce; issuing it
    anyway double-counts, which in bf16 overflows rather than merely drifting."""
