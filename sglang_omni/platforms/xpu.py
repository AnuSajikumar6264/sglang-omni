...
class XPUOmniPlatform(OmniPlatform):
    ...
    def moe_router_logits_dtype(self, gate_dtype: "torch.dtype") -> "torch.dtype":
        # SGLang routes XPU top-k through sgl-kernel's fused softmax kernel
        # whenever top_k <= 8 and experts <= 256, and that kernel registers only
        # the half dtypes. Keep the gate's own dtype: the gate computed there, so
        # the fp32 copy other platforms take adds nothing to undo.
        return gate_dtype
