...
class DllmScheduler:
    ...
    def __init__(
        ...
        *,
        ...
        self._max_requests_per_round = current_platform.dllm_max_requests_per_round()
    ...
    def _schedule_next_batch(self) -> ScheduleBatch | None:
        ...
        if not staging_no_token:
            for req in self._waiting_queue:
                if (
                    self._max_requests_per_round is not None
                    and len(adder.can_run_list) >= self._max_requests_per_round
                ):
                    break
