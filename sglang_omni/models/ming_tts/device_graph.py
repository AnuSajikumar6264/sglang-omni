...
# Every accelerator names its graph type after itself, and none of them share a
# base class, so the type is found by name rather than by isinstance.
_GRAPH_TYPE_NAMES = ("CUDAGraph", "XPUGraph")


def accelerator_module(device: torch.device) -> Any | None:
...
def accelerator_graph_class(device_module: Any) -> type:
...
def _accepts_capture_error_mode(device_module: Any) -> bool:
    try:
